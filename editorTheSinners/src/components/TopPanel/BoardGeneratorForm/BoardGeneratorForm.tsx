...
const BoardGeneratorForm: ComponentType<ClassProp> = ({ className }) => {
    ...
    const handleClick = (event: MouseEvent) => {
        ...
        let content =
            editorContext?.boardMatrix.matrix !== undefined
                ? editorContext.boardMatrix.matrix
                : "";
    ...
    return (
        <form className={className}>
            <Textarea {...textaAreaProps} />
            <button onClick={(event: MouseEvent) => handleClick(event)}>
                Generate
            </button>
        </form>
    );
